...
let suppressSuccessToasts = false;

export const setSyncSuccessToastsSuppressed = (suppressed) => {
  suppressSuccessToasts = !!suppressed;
};
...
export const processSyncQueue = async ({ silent = false } = {}) => {
  ...
  try {
    ...
    if (!silent && totalSynced > 0 && totalFailed === 0 && stats.pending === 0 && !suppressSuccessToasts) {
      Toast.show({
        type: 'success',
        text1: 'Sync Complete',
    ...
    } else if (!silent && suppressSuccessToasts && stats.pending > 0 && totalFailed === 0) {
      Toast.show({
        type: 'warning',
        text1: 'Sync Pending',
        text2: `${stats.pending} item${stats.pending !== 1 ? 's' : ''} still waiting to sync`,
        visibilityTime: 3000,
        position: 'top',
      });
