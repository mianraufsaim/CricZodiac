...
function _syncResult($p,$a,$d){ $p->prepare("INSERT INTO match_results (local_id,match_local_id,winner_team_local,loser_team_local,result_type,margin,margin_type,team_a_score,team_b_score,player_of_match_local,result_text,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,NOW()) ON DUPLICATE KEY UPDATE result_text=VALUES(result_text)")->execute([$d['id'],$d['match_id'],$d['winner_team_id']??null,$d['loser_team_id']??null,$d['result_type']??'win',$d['margin']??0,$d['margin_type']??'runs',$d['team_a_score']??null,$d['team_b_score']??null,$d['player_of_match']??null,$d['result_text']??null]); return true; }
